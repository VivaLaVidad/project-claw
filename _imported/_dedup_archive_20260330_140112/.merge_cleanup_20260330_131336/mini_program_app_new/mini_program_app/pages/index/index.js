 ...
 * 首页：询价表单 + 位置获取 + 健康检测
 */
const { ensureToken, loginAndGetToken, healthCheck, getOnlineMerchants, requestTrade, BASE_URL } = require('../../utils/api');
...
Page({
  ...
  onLoad() {
    const app = getApp();
    const cid = app.globalData.clientId || wx.getStorageSync('claw_client_id') || '';
    ensureToken(cid).catch(() => {});
  ...
  async _refreshMerchants() {
    ...
    for (let i = 0; i < 3; i++) {
      try {
        ...
        this.setData({ onlineMerchants: (res && res.online_merchants) || 0 });
        return;
      } catch (e) {
        if (i === 2) {
          const detail = (e && (e.detail || e.errMsg || e.message)) || 'network_error';
